...
**16:14 Eystein Magnus Hansen:** What happens if the delegation vote and the user votes differ (if a user can both delegate their vote but also vote themself)? Will the user’s vote then override delegation vote?\
__**Answer:** In the first iteration, it will definitely NOT be possible to override the dRep’s’ vote on an individual proposal that you care about; but for future iterations, we can discuss it further. We need to understand why people would want to do that and why it would be useful.

**17:42 Tomi Astikainen** Can a dRep delegate their own voting power to another dRep?\
**Answer:** No; that breaks the purpose of dReps.

**18:07 Zoé Selina:** Do voters have to choose between delegating, and voting themselves; or can they delegate a percentage of their voting power?\
...
**21:33 Zoé Selina**  There are many voices against downvoting. Why is this just adopted to dReps without having the conversation?\
...
**23:40 Sebastian Pereira:** If a dRep fails to vote for the minimum amount of proposals, do the delegators still receive rewards?\
**Answer:** Yes. The dRep will not receive rewards, but their delegators will; and their voting power will be counted.
...
**25:29** Will there be an opportunity to set custom tags?\
...
**28:53** Voting centre\
&#x20;\- 29:05 Landing page\
&#x20;\- 29:19 List of dReps  – the info comes from data captured during dRep registration. Order is randomised: there is no hierarchy. Oversaturated dReps  are filtered out.

### **Clarification on saturation and reward calculation:**

**31:16** The voting-power cap is 1% of the total **voting** stake – i.e. the total amount of ADA used to vote in a particular fund. But dRep rewards are capped at 1% of the total **delegated** stake. These are two different numbers. \
<mark style="color:red;">For example,</mark> if there is 1 billion ADA total voting stake (i.e. the total amount of ADA used in a fund), but only 100 million of that was delegated, then the maximum voting power that a dRep can have is 10 million ADA (1% of the 1 billion total), but a dRep only needs to have 1 million ADA to earn their full rewards, because it's 1% of the delegated total, 100 million, rather than 1% of a billion.\
...
**36:59** There are two different voting plans: one for dReps, and another for direct voters; so once the vote is complete, the dRep voting plan is made public, and the dRep votes become a matter of public record.

**37:27** As a dRep, how do I account for my own wallet's voting power?\
**Answer:** See slide #17 - the breakdown between staked and delegated ADA is displayed in your profile.

**38:17 Jason Toevs:** Are these wallet integration requirements open source?\
...
**38:58  Alejandro Gelves Gelves:** Is it possible to have whale dReps, that get max reward, even though no one delegates to them?\
**Answer:** Yes. If you are a whale, you could create 2 wallets, register one as a dRep, and delegate to yourself. But depending how big a whale you are, you might therefore lose some of your voting influence, because as a dRep your voting influence is capped at 1%. You would still have to meet the minimum requirements of voting on 60 proposals and 20 Challenge proposals; so we would be paying a whale to work, which is not a bad thing.
...
**Answer:** No; there’s no distinction. Your voting power is your voting power, and that's that.

**40:56 Zoé Selina** On how public the votes are – are there 2 stages of voting, one for dReps and one for direct voters?\
...
**42:33 Sebastian Pereira**: In the case of failure by the dRep (for example, if the dRep fails to vote for enough proposals), how will the rewards for delegators be calculated?\
...
**Answer**: Yes. Voting history is one of our future plans, and we hope to show it in a future workshop like this. Obviously, it will be an opt in; you will be able to choose whether you want your voting history to be available to you.
...
**Answer:** As mentioned earlier, it’s randomised. Oversaturated dReps are not shown initially.
...
**52:32 Darlington Wleh** Will the open APIs be published on the same timeline as the UI here?\
...
**54:21 Kenric Nelson:** if a direct delegation goes over 1%, can people still delegate to them?\
...
**Answer:** It’s separate – direct voting is different and separate.
...
**57:54 Joanna** Can every wallet choose to delegate their entire voting power to 10 different dReps? Or do they have to allocate their voting power **across** 10 different dReps?\
...
**58:27 Joanna** But then, if two different dReps voted on the same proposal, that's doubling my voting power, whereas now I can only vote on one proposal?\
...
**Answer:** For votes from before you became a dRep: one requirement to become a dRep is that you need to have voted in a previous fund. To maintain privacy e.g. on how much ADA you hold, you can create a new wallet as a dRep; so we will validate that you voted in a previous fund via the proof of life review, a private meeting which we still need to define. For votes in future Funds, we want to let users see a dRep’s previous voting history, so we are planning that for a future iteration.\
...
**Answer:** For voter rewards, it will be the same as if they voted from one wallet with all their ADA in it, because thats how voter rewards are calculated. If they were to try to become more than one dRep, they would be filtered out at the Proof of Life stage.

**1:02:52 Eystein Magnus Hansen** Will it be frowned upon for dReps to make public how they plan to vote ?\
**Answer:** It’s a matter of transparency. A dRep says in their profile what types of projects they will vote for, and people will delegate to them based on that. If they then vote for something different, that could cause problems with people who delegated to them. But their rationales should explain why they voted as they did. People need to know they can trust you as a dRep to use _their_ voting power the way you said you would.

**1:04:22 Jason Toevs** Are the delegated wallet addresses made public with the dRep Votes after the Tally?\
**Answer** If you're voting directly, your vote is kept private. If you've delegated, you're not the one casting the vote; the dRep is doing it on your behalf, and their wallet is made public.
...
**Answer:** That's not a parameter that we've discussed. For the first iteration, a good success metric could be that 20% of all voting stake was delegated. But we can’t predict how people will behave; and we want the community to also define some of these metrics.
...
**Answer:** As a direct voter you only need to cast one vote to get your full voting reward. Now, with a dRep, if they don't cast any vote, do the delegates get the reward? I'm not sure they do.  I think it's just a mechanical rather than a  philosophical rule.\
...
**1:21:23 Tomi Astikainen:** What are the implications if I create 100 different wallets and hire 100 people to do the dRepping for me; they do the PoL but essentially it's one big team against the world?\
...
**1:23:06 Andreas Sosilo** So are we not using the Catalyst voting app any more? And do we vote using this new dashboard in Fund 10?\
**Answer:** Our goal for Fund 10 is for dReps  to be able to use it, to  vote and to manage their profiles; and for delegators to use it to find dReps and delegate to them. The third element is the active voter – for them, the Catalyst voting app, and QR and PIN codes, will remain the primary voting method in fund 10, unless they are brave enough to risk a beta app. We don't want to stop anyone artificially from using the app; but we are not recommending general public use for Fund 10. So the main focus of the Voting Centre for Fund 10 is for dReps, and we hope it will be a real alternative for everyone in Fund 11.

**1:25:31 Eystein Magnus Hansen** On API, is there a protection mechanism in place against blocking voting registration on the API; DDoS (distributed denial-of-service) attack and such?\
...
**1:26:40 Voltaire Valdellon** If wallets are static and public, what if a wallet gets compromised in some way?  Is there a workaround?\
